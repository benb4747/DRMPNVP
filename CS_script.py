...
def test_algorithms(inp):
    ...
    headers = [index, rep, num_reps, mu_0, sig_0, T, W, tuple(w), p, h, b,
    PWL_gap, n_pts, CS_tol, timeout, N, alpha]
    ...
    if AS.base_set == "T.O.":
        for f in [count_file, results_file]:
            with open(f, "a") as myfile:
                myfile.write("Input %s timed out while constructing base AS. \n" %index)
            return
    AS.construct_confidence_set()
    if AS.confidence_set_full == "T.O.":
        for f in [count_file, results_file]:
            with open(f, "a") as myfile:
                myfile.write("Input %s timed out while constructing confidence set. \n" %index)
            return
    AS.compute_extreme_distributions()
    if AS.extreme_distributions == "T.O.":
        for f in [count_file, results_file]:
            with open(f, "a") as myfile:
                myfile.write("Input %s timed out while constructing extreme set. \n" %index)
            return
    AS.reduce()
    if AS.reduced == "T.O.":
        for f in [count_file, results_file]:
    ...
    if num_dists <= 1:
        with open(count_file, "a") as myfile:
            myfile.write("Input %s only had %s distribution. Skipping! \n" %(index, num_dists))
        return
    
    AS_tt = AS.time_taken
    headers += [len(AS.confidence_set_full), len(AS.reduced)]
    DRO_problem = DRMPNVP(T, W, w, p, h, b, PWL_gap, timeout-AS_tt, 
    ...
    res_list = headers + [
        tuple(np.round(PWL_q, 3)), tuple([tuple(i) for i  in np.round(PWL_worst, 3)]), np.round(PWL_obj, 5), 
        tuple([tuple(i) for i  in np.round(PWL_true_worst, 3)]), np.round(PWL_true_worst_obj, 5), 
        PWL_tt, int(PWL_TO), PWL_true_obj,
        tuple(np.round(CS_q, 3)), tuple([tuple(i) for i  in np.round(CS_worst, 3)]), 
        np.round(CS_obj, 5), 
        tuple([tuple(i) for i  in np.round(CS_true_worst, 3)]), 
        np.round(CS_true_worst_obj, 5), CS_tt, int(CS_TO), CS_reason, CS_true_obj,
        tuple(np.round(fd_q, 3)), tuple([tuple(i) for i  in np.round(X.mle, 3)]), np.round(fd_obj, 5), 
        tuple([tuple(i) for i  in np.round(fd_true_worst, 3)]), 
        np.round(fd_true_worst_obj, 5), fd_tt, fd_true_obj
...
names = ["ind", "rep", "num_reps", "mu_0", "sig_0", "T", "W", "w", "p", "h", "b", 
         "PWL_gap", "n_pts", "CS_tol", "timeout", "N",
...
if T == 2 and continuing:
    with open(count_file, "a") as myfile:
        myfile.write("About to start solving the %s instances that didn't finish solving before. \n" %len(test))
