...
def test_algorithms(inp):
    (index, rep, num_MLE, mu_0, sig_0, T, W, w, p, h, b, 
     PWL_gap, n_pts, CS_tol, solver_timeout, solver_cores, N, 
     alpha) = inp
    
    headers = [index, rep, num_MLE, mu_0, sig_0, T, W, tuple(w), p, h, b,
    PWL_gap, n_pts, CS_tol, solver_timeout, N, alpha]
    ...
    headers += [1, 1]
    DRO_problem = DRMPNVP(T, W, w, p, h, b, PWL_gap, solver_timeout, 
    ...
    PWL_omega0_q, PWL_omega0_obj, PWL_omega0_worst, PWL_omega0_tt, PWL_omega0_TO = PWL_omega0_sol
    ...
    fd_MLE_problem = MPNVP(T, W, w, p, h, b, PWL_gap, solver_timeout, 
    ...
    fd_omega0_problem = MPNVP(T, W, w, p, h, b, PWL_gap, solver_timeout, 
                   solver_cores, dist, omega_0, budget_tol=1e-6, 
                   tau=0.5, alpha_min=1e-100, alpha_init=0.1)
    s=time.perf_counter()
    ...
    fd_omega0_true_obj = np.round(fd_omega0_problem.cost_function(fd_omega0_q, omega_0), 5)
    
    with open(count_file, "a") as myfile:
        myfile.write("Finished solving input %s replication %s. \n" %(index, rep))

    res_list = headers + [
        tuple(np.round(PWL_omega0_q, 3)), tuple([tuple(i) for i  in np.round(PWL_omega0_worst, 3)]), 
        np.round(PWL_omega0_obj, 5), 
        PWL_omega0_tt, int(PWL_omega0_TO), PWL_omega0_true_obj,
        
        tuple(np.round(PWL_MLE_q, 3)), tuple([tuple(i) for i  in np.round(PWL_MLE_worst, 3)]), 
        np.round(PWL_MLE_obj, 5), 
        PWL_MLE_tt, int(PWL_MLE_TO), PWL_MLE_true_obj,
        
        tuple(np.round(fd_omega0_q, 3)), tuple([tuple(i) for i  in np.round(X.mle, 3)]), 
        np.round(fd_omega0_obj, 5), 
        fd_omega0_tt, fd_omega0_true_obj,
        
        tuple(np.round(fd_MLE_q, 3)), tuple([tuple(i) for i  in np.round(X.mle, 3)]), 
        np.round(fd_MLE_obj, 5), 
        fd_MLE_tt, fd_MLE_true_obj
...
names = ["ind", "rep", "num_MLE", "mu_0", "sig_0", "T", "W", "w", "p", "h", "b", 
         "PWL_gap", "n_pts", "CS_tol", "solver_timeout", "N", "alpha", "num_dists_full",
         "num_dists_reduced",
         "PWL_omega0_q", "PWL_omega0_worst", "PWL_omega0_obj", 
         "PWL_omega0_tt", "PWL_omega0_TO", "PWL_omega0_true_obj",
         "PWL_MLE_q", "PWL_MLE_worst", "PWL_MLE_obj", 
         "PWL_MLE_tt", "PWL_MLE_TO", "PWL_MLE_true_obj",
         "fd_omega0_q", "fd_omega0_worst", "fd_omega0_obj", "fd_omega0_tt", 
         "fd_omega0_true_obj",
         "fd_MLE_q", "fd_MLE_worst", "fd_MLE_obj", "fd_MLE_tt", "fd_MLE_true_obj"
]  
...
if continuing:
    with open(count_file, "a") as myfile:
        myfile.write("About to start solving the %s instances that didn't solve before. \n" %len(test))
...
if not continuing:
    ...
    with open(count_file, "a") as myfile:
        myfile.write("About to start solving %s instances with the repeated sampling approach. \n" %len(repeated_inputs))
