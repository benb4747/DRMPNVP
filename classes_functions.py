...
def mu_sig_combos(mu_0_range, sig_0_range, T):
    return [(mu_0, sig_0) for mu_0 in list(it.combinations_with_replacement(mu_0_range, T))
...
class ambiguity_set():
    ...
    def construct_base_set(self):
        ...
        mu_hat, sig_hat = omega_hat
        mu_CIs = [(mu_hat[t] - z * (sig_hat[t] / np.sqrt(N)), mu_hat[t] + z * (sig_hat[t] / np.sqrt(N)))
                  for t in range(self.demand.T)]
        sig_CIs = [(sig_hat[t] - z * (sig_hat[t] / np.sqrt(2 * N)), sig_hat[t] + z * (sig_hat[t] / np.sqrt(2*N)))
                 for t in range(self.demand.T)]

        mu_CIs_disc = [[mu_CIs[t][0] + (j / (self.n_pts - 1)) * (mu_CIs[t][1] - mu_CIs[t][0]) 
                        for j in range(self.n_pts)]
                       for t in range(self.demand.T)]

        sig_CIs_disc = [[sig_CIs[t][0] + (j / (self.n_pts - 1)) * (sig_CIs[t][1] - sig_CIs[t][0]) 
                        for j in range(self.n_pts)]
                       for t in range(self.demand.T)]
    ...
    def construct_confidence_set(self):
        ...
        if self.demand.dist == "normal":
            ...
            for omega in self.base_set:
                if norm_conf_val(self.demand.N, self.demand.T, omega, self.demand.mle) <= chi:
    ...
    def compute_extreme_distributions(self):
        ...
        if self.demand.dist == "normal":
            ...
            for o in self.confidence_set_full:
                if o[0] not in M: M.append(o[0])

            ext_1 = []
            for mu in M:
                max_sig_sum = max([sum(o[1]) for o in self.confidence_set_full if o[0] == mu])
                ext_1 += [o for o in self.confidence_set_full if o[0] == mu and sum(o[1]) == max_sig_sum]

            S = []
            for o in ext_1:
                if o[1] not in S: S.append(o[1])

            ext_2 = []
            for sig in S:
                mu_max = [max([mu[t] for (mu, s) in ext_1 if s == sig]) for t in range(self.demand.T)]
                mu_min = [min([mu[t] for (mu, s) in ext_1 if s == sig]) for t in range(self.demand.T)]

                ext_2 += [(m, s) for (m, s) in ext_1 if s==sig and 
                         np.any(np.array([m[t] in [mu_max[t], mu_min[t]] for t in range(self.demand.T)]))
                         ]
...
class DRMPNVP():
    ...
    def __init__(self, T, W, w, p, h, b, gap, solver_timeout, 
    ...
    def cost_function(self, q, omega):
        if self.dist == "normal":
            ...
            alpha = [sum([mu[k] - q[k] for k in range(t + 1)]) / s[t] for t in range(self.T)]

            obj = sum([((self.a[t] * norm.cdf(alpha[t]) - self.h) * sum([mu[k] - q[k] for k in range(t + 1)])
                + self.a[t] * norm.pdf(alpha[t]) * s[t] + q[t] * self.w[t] - self.p * mu[t])
                       for t in range(self.T)])
            return obj
    ...
    def PWL_NLpart(self, omega, alpha, alpha_pts, t):
        ind = [i for i in range(len(alpha_pts) - 1) if 
               alpha_pts[i] <= alpha and alpha <= alpha_pts[i+1]][0]
        return (((alpha - alpha_pts[ind+1]) / (alpha_pts[ind] - alpha_pts[ind+1])) * 
                self.nonlinear_part(omega, alpha_pts[ind], t) +
                 ((alpha - alpha_pts[ind]) / (alpha_pts[ind+1] - alpha_pts[ind])) * 
                self.nonlinear_part(omega, alpha_pts[ind+1], t))
    ...
    def build_model(self, ambiguity_set):
        ...
        enable_print()
        if self.dist == "normal":
            alpha = m.addVars([(i, t) for i in range(len(ambiguity_set)) 
                     for t in range(self.T)], vtype=GRB.CONTINUOUS, name="alpha", 
                      lb=-GRB.INFINITY)
            NL_part = m.addVars([(i, t) for i in range(len(ambiguity_set)) 
                             for t in range(self.T)], vtype=GRB.CONTINUOUS, name="NL_part", 
                              lb=-GRB.INFINITY)
            
            alpha_min = np.array([[sum([mu[k] - (self.W / self.w[k]) for k in range(t+1)]) / 
                           np.sqrt(sum([sig[k]**2 for k in range(t+1)]))
                           for t in range(self.T)]
                          for (mu, sig) in ambiguity_set])
            alpha_max = np.array([[sum([mu[k] for k in range(t+1)]) / np.sqrt(sum([sig[k]**2 for k in range(t+1)]))
                                   for t in range(self.T)]
                                  for (mu, sig) in ambiguity_set])
            alpha_pts = np.array([[np.arange(np.floor(alpha_min[i, t]), 
            ...
            for i in range(len(ambiguity_set)):
                ...
                s = [np.sqrt(np.sum([sig[k]**2 for k in range(t+1)])) for t in range(self.T)]
                for t in range(self.T):
                    NL_pts = [self.nonlinear_part((mu, sig), alpha, t) for alpha in alpha_pts[i, t]]
                    m.addGenConstrPWL(alpha[i, t], NL_part[i, t], alpha_pts[i, t], NL_pts, "NLconstr_(%s,%s)" %(i,t))
                    ...
                    m.addConstr(alpha[i, t] == gp.quicksum(mu[k] - q[k] for k in range(t+1)) / s[t], 
                                name="alpha_(%s, %s)_constraint" %(i,t))

                m.addConstr(dummy >= gp.quicksum(NL_part[i, t] + q[t] * self.w[t] 
                                                 - self.p * mu[t] 
                                                 for t in range(self.T)))
    ...
    def solve_model(self, m, var, t_build, TO, ambiguity_set):
        ...
        if m.Status in [GRB.OPTIMAL, GRB.TIME_LIMIT] and m.solCount > 0:  
            ...
            NL_sol = np.array(m.getAttr("x", NL_part).values()).reshape(len(ambiguity_set), self.T)
    ...
    def PWL_solve(self, ambiguity_set):
        ...
        [q_sol, obj, worst, tt, TO] = self.solve_model(m, var, t_build, TO, ambiguity_set)
    ...
    def CS_solve(self, omega_0, verbose=False, eps=0.01):
        ...
        while k < k_max:
            ...
            [q, alpha, dummy, NL_part] = var
            [q_k, obj_master, o_master, tt_master, TO] = self.solve_model(m, var, t_build, TO, Omega_k)
...
class MPNVP(DRMPNVP):
    ...
    def __init__(self, T, W, w, p, h, b, gap, solver_timeout, 
                 solver_cores, dist, omega, budget_tol, tau,
                 alpha_min=1e-100, alpha_init=0.1):
        
        super().__init__(T, W, w, p, h, b, gap, solver_timeout, 
                 solver_cores, dist, [omega])
    ...
    def KKT_solution(self, lam):
        if self.dist == "normal":
            ...
            if 0 not in self.zero_days:
                pr = (Decimal(self.b) - Decimal(1 + lam) * 
                      Decimal(self.w[0] - self.w[1])) / Decimal(self.h + self.b)
                sol[0] = Decimal(
                    norm.ppf(q=np.float64(pr),
                             loc=mu[0], scale=np.float64(Decimal(np.sqrt(var[0]))))
                )
            ...
            for t in range(1, self.T - 1):
                if t not in self.zero_days:
                    ...
                    pr = (Decimal(self.b) - Decimal(1 + lam) * 
                          Decimal(self.w[t] - self.w[t+1])) / Decimal(self.h + self.b)

                    sol[t] = (Decimal(
                        norm.ppf(q=np.float64(pr), 
                                 loc=sum(mu[:t+1]), scale=np.float64(Decimal(np.sqrt(sum(var[:t+1]))))
                                )
                    )
                    - Decimal(sum(sol[:t]))
                             )
            ...
            if self.T - 1 not in self.zero_days:
                pr = (Decimal(self.b) - Decimal(1 + lam) * 
                      Decimal(self.w[-1]) + Decimal(self.p)) / Decimal(self.h + self.b + self.p)
                sol[self.T - 1] = (
                        Decimal(
                        norm.ppf(q=np.float64(pr), 
                                 loc=sum(mu), scale=np.float64(Decimal(np.sqrt(sum(var)))))
                        )
    ...
    def alfares_solve(self):
        ...
        [self.h, self.b, self.p, self.W] = [float(x) for x in [self.h, self.b, self.p, self.W]]
        ...
        while not opt:
            ...
            if sum(self.w * q) - self.W > self.budget_tol:
                ...
                a = 0 
                lam_UBs = ([Decimal(self.b) / Decimal(self.w[i] - self.w[i+1]) 
                            - Decimal(1) for i in range(self.T - 1)] 
                           + [(Decimal(self.b) + Decimal(self.p)) 
                              / Decimal(self.w[-1]) - Decimal(1)])
                ...
                while not done:
                    ...
                    if lam_next == lam_old or (alpha == self.alpha_min and np.all(q_next == q_temp)):
                        break

                    if ((len(np.where(q_next < 0)[0]) == 1 and np.all(np.isfinite(q_next))
                        ...
                        if alpha_next <= self.alpha_min or lam_old + alpha_next == lam_old:
                    ...
                    if ((abs(Decimal(self.W) - Decimal(sum(q_next * self.w))) <= self.budget_tol 
                        and np.all(q_next >= 0)) or (np.any(q_next < 0))):
                        # solution is optimal
                        done = True
                ...
                elif np.any(q < 0) or Decimal(sum(self.w * q)) - Decimal(self.W) > Decimal(self.budget_tol):
                    LBs = []
                    q_LBs = []
                    for day in days:
                        self.zero_days.append(day)
                        days_temp = [t for t in range(self.T) if t not in self.zero_days]
                        if len(days_temp) > 1:
